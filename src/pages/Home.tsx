...
const Home: React.FC = () => {
  ...
  const FishSVG = ({ color }: { color: string }) => (
    <svg width="225" height="120" viewBox="0 0 150 80" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet">
      {/* Fish Tail */}
      <path d="M 20 40 L 0 25 L 5 40 L 0 55 Z" fill={color} className="fish-fin" />
      
      {/* Fish Body */}
      <ellipse cx="75" cy="40" rx="50" ry="30" fill={color} className="fish-body" />
      
      {/* Top Fin */}
      <path d="M 70 10 L 65 0 L 75 5 L 85 0 L 80 10 Z" fill={color} className="fish-fin" />
      
      {/* Bottom Fin */}
      <path d="M 60 70 L 55 80 L 65 75 L 75 80 L 70 70 Z" fill={color} className="fish-fin" />
      
      {/* Mouth */}
      <path d="M 120 40 Q 125 42 120 44" stroke={color} strokeWidth="2" fill="none" />
      
      {/* Eye - Always render last to be on top */}
      <circle cx="100" cy="35" r="8" fill="white" stroke="black" strokeWidth="1" className="fish-eye" />
  ...
  return (
    <div className='flex flex-col gap-6 max-w-7xl mx-auto px-8 py-12 text-center min-h-[calc(100vh-5rem)]'>
      <h1 className='text-5xl md:text-4xl sm:text-3xl font-bold mb-6 text-white font-["Bangers"]'>
        Welcome to GoFishTalent
      </h1>
      ...
      <div className='mt-8 flex flex-wrap justify-center gap-6'>
        {musicians.map((musician) => (
          <div 
            ...
            onClick={() => handleFishClick(musician.route)}
          >
            <img src={musician.image} alt={musician.name} className='w-10 h-10 rounded-full border-2 border-white' />
